...
if __name__ == "__main__":
    wide_format_dir = Path("code/Task3/wide_format_dataset/NEW_final_wide_format_dataset").resolve().absolute()
    high_anx_df_path = Path("code/Task1/individual_tfmn/high_anxiety_personas.csv").resolve().absolute()
    high_anx_df = pd.read_csv(high_anx_df_path)
        ...
        if dir.is_dir() and dir.name != "out": # skip the out folder itself to avoid looping over generated data
            for file in dir.iterdir():
                if file.suffix == ".csv": # ensure we only process CSVs
                    ...
                    df = pd.read_csv(file)
                    df = df.merge(high_anx_df, on="run_id", how="inner")
                    folder_path = wide_format_dir
                    
                    # USE THE STEM (the name without .csv) to prevent FileExistsError
                    file_name_clean = str(file.stem) + "_high_anxiety_"
