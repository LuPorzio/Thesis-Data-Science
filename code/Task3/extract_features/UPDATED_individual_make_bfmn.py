...
def build_bfmn_from_edgelist(csv_path, draw_fmnt: bool = False):
    ...
    # 4. Add nodes and categorize by valence
...
def extract_network_features(G, df_edgelist, model_name, run_id):
    ...
    # 8. Topic centralities (Betweenness)
    topic_centralities = nx.betweenness_centrality(G)

    # 9. Closeness centrality
    closeness_centralities = nx.closeness_centrality(G)
    
    # 10. HUBs (HITS Algorithm scores)
    try:
        hub_scores, _ = nx.hits(G, max_iter=500)
    except Exception:
        hub_scores = None

    # 11. Statistical Degree Hubs (Nodes with degree > mean + 1 std deviation)
    degree_values = list(degrees_dict.values())
    if degree_values:
        mean_degree = np.mean(degree_values)
        std_degree = np.std(degree_values)
        degree_hubs = [node for node, deg in degrees_dict.items() if deg > (mean_degree + std_degree)]
    else:
        degree_hubs = []
    ...
    return {
        ...
        "closeness_centralities": str(closeness_centralities),
        "hubs_scores_hits": str(hub_scores),
        "degree_hubs": str(degree_hubs)
    }
